...
<body>\
	...
	<div class='main'>\
		...
		<div class='container'>\
		<form action='' method='post'  class='search' id='fo'>\
				<label>CODE: </label><input type='search' name='search' id='search-inp' required><button><i class='fas fa-search'></i></button>\
			</form>\
			<table id='dataTable'>\
				<tr>\
					<td>code</td>\
        ...
        if(!con)
        {
            ...
            fscanf(stdin, "%s", &data);
				...
				taille = sizeof("SELECT * FROM tclient WHERE CODE_C = ''") +2;
	            query_array = malloc(taille + 1);
	            if(code != NULL)
	            {
	            	sprintf(query_array, "SELECT * FROM tclient WHERE CODE_C = %s;", code);
				} else {
